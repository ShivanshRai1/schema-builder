...
const KIND_WORDS: Record<string, ComponentKind> = {
  ...
  varicap: "CVAR",
  vsource: "V", voltage: "V", voltagesource: "V", supply: "V", v: "V",
  battery: "BATTERY", vdc: "BATTERY", dcvoltage: "BATTERY",
  vac: "VAC", acvoltage: "VAC", sine: "VAC", sinusoid: "VAC",
  isource: "I", current: "I", currentsource: "I", idc: "I", i: "I", dccurrent: "I",
  iac: "IAC", accurrent: "IAC",
  pulse: "VPULSE", vpulse: "VPULSE", pulsegen: "VPULSE", generator: "VPULSE",
  diode: "D", diodes: "D", d: "D",
  zener: "DZ", zenerdiode: "DZ", zenerdiodes: "DZ", dz: "DZ",
  schottky: "DS", schottkydiode: "DS", ds: "DS",
  led: "LED", leds: "LED",
  tvs: "DTVS", tvsunidirectional: "DTVS", tvsbidirectional: "DTVSBI",
  mosfet: "NMOS", nmos: "NMOS", pmos: "PMOS", transistor: "NMOS", fet: "NMOS",
  emos: "NMOS", dmos: "NMOS_D",
  jfet: "NJFET", njfet: "NJFET", pjfet: "PJFET",
  sic: "SICMOS", sicmosfet: "SICMOS", gan: "GANHEMT", ganhemt: "GANHEMT", igbt: "IGBT",
  bjt: "NPN", npn: "NPN", pnp: "PNP", ujt: "UJT", unijunction: "UJT",
  transformer: "XFMR", xfmr: "XFMR", xfrmr: "XFMR",
  thyristor: "SCR", scr: "SCR", scs: "SCS", triac: "TRIAC", diac: "DIAC",
  gto: "GTO", photothyristor: "SCR_PH", sidac: "SIDAC",
  and: "AND", or: "OR", nand: "NAND", nor: "NOR", xor: "XOR", xnor: "XNOR",
  not: "NOT", inverter: "NOT",
  srff: "SRFF", jkff: "JKFF", tff: "TFF", dff: "DFF",
  flipflop: "DFF", latch: "SRFF",
  opamp: "OPAMP", operationalamplifier: "OPAMP",
  diffamp: "DIFFAMP", differential: "DIFFAMP", differentialamplifier: "DIFFAMP",
  eamp: "OPAMP", erroramp: "OPAMP", amplifier: "OPAMP",
  comparator: "COMP", comp: "COMP",
  crystal: "XTAL", oscillator: "XTAL",
  driver: "GATEDRV", gatedriver: "GATEDRV", gatedrv: "GATEDRV",
  constant: "MATH_CONST", sum: "MATH_SUM", product: "MATH_PROD",
  multiply: "MATH_PROD", multiplier: "MATH_PROD", divide: "MATH_PROD",
  gain: "MATH_GAIN", relational: "MATH_REL", logical: "MATH_LOGIC",
...
export function interpret(input: string): InterpretResult {
  ...
  return {
    ops: [],
    reply:
      'I didn’t catch that. Try: "add 10k resistor", "set R1 value 4.7k", "set D1 model Foo", "connect R1 to C1", or "remove C1".',
  };
}
...
function matchSet(text: string): InterpretResult | null {
  // "set D1 model to SM8S36A" / "set D1 to model Foo" / "use model Foo for D1"
  ...
  const patterns: RegExp[] = [
    ...
    // give R1 a value of 4.7k / give R1 model Foo
    new RegExp(
...
/** Explicit model / part / subckt phrases (before value catch-alls). */
function matchSetModel(text: string): InterpretResult | null {
  const patterns: RegExp[] = [
    // set D1 to model Foo / change D2 model to Bar
    new RegExp(
      `^(?:${SET_VERBS})\\s+([A-Za-z]+\\d+)\\s+(?:to\\s+)?(?:${MODEL_WORDS})\\s+(?:to\\s+|as\\s+|==?\\s*)?(.+)$`,
      "i",
    ),
    // set model of D1 to Foo / change the model for D2 to Bar
    new RegExp(
      `^(?:${SET_VERBS})\\s+(?:the\\s+)?(?:${MODEL_WORDS})\\s+(?:of\\s+|for\\s+|on\\s+)?([A-Za-z]+\\d+)\\s+(?:to\\s+|as\\s+|==?\\s*)(.+)$`,
      "i",
    ),
    // use model Foo on D1 / use Foo for D1 model
    new RegExp(
  ...
  for (const re of patterns) {
    ...
    return {
      ...
      reply: `Ready to set ${refdes} model = ${value}.`,
    };
  }
  return null;
}
