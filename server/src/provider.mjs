      ...
      items: {
        ...
        properties: {
          ...
          key: {
            type: "string",
            description:
              'For setParam: use "value" for R/L/C/V/I passives and sources; use "model" for diodes/TVS/MOSFET/IGBT/GaN/BJT part names (never put the model name in key).',
          },
          value: {
            type: "string",
            description:
              'For setParam: the new value (e.g. "4.7k", "100n") or model/subckt name (e.g. "SM8S36A", "XFD11K54CA", "DZEN").',
          },
          params: {
            type: "object",
            description:
              'Optional initial params for addComponent, e.g. {"value":"10k"} or {"model":"DZEN"}.',
          },
...
function buildSystemAndUser(message, context, history) {
  const system = [
    ...
    "2. setParam keys:",
    '   - Passives / sources (R, L, C, V, I, …): key MUST be "value" (e.g. 4.7k, 100n, 12).',
    '   - Semiconductors (D, DZ, DS, LED, DTVS, DTVSBI, MOSFET, IGBT, GaN, BJT, …): to change the SPICE model/part, key MUST be "model" and value is the model/subckt name.',
    '   - Do NOT invent keys like "resistance" or "part" — use "value" or "model" only for those cases.',
    ...
    "5. Prefer ops that match existing refdes from the component list (R1, D1, …). For new parts use addComponent.",
    "",
    "Edit examples:",
    'User: "add resistor" → ops:[{"type":"addComponent","kind":"R"}]',
    'User: "add 100n capacitor" → ops:[{"type":"addComponent","kind":"C","params":{"value":"100n"}}]',
    'User: "set R1 to 10k" / "change R1 to 4.7k" → ops:[{"type":"setParam","refdes":"R1","key":"value","value":"10k"}]',
    'User: "set D1 model to SM8S36A" / "set D2 model XFD11K54CA" → ops:[{"type":"setParam","refdes":"D1","key":"model","value":"SM8S36A"}]',
    'User: "use model FooBar for D1" → ops:[{"type":"setParam","refdes":"D1","key":"model","value":"FooBar"}]',
    'User: "change R1 to 4.7k and connect it to C1" → two ops: setParam (value) + connectPins',
